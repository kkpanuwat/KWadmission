            ...
            if (isset($_GET["btn_s"])) {
                ...
                if ($_GET["op_s"] == "fname") {
                    $sql = "SELECT *
                        ...
                        WHERE fname ='$user_get' or fname like '%$user_get' or fname like '%$user_get%' or fname like '$user_get%' ";
                }
                if ($_GET["op_s"] == "lname") {
                    $sql = "SELECT *
                        FROM student
                        INNER JOIN application_form ON student.id_card = application_form.id_card
                        INNER JOIN examination_card ON examination_card.barcode_id = application_form.id_card 
                        WHERE lname ='$user_get' or lname like '%$user_get' or lname like '%$user_get%' or lname like '$user_get%'";
                }
                if ($_GET["op_s"] == "id_card") {
                    $sql = "SELECT *
                        ...
                        INNER JOIN application_form ON student.id_card = application_form.id_card
                        INNER JOIN examination_card ON examination_card.barcode_id = application_form.id_card 
                        WHERE student.id_card ='$user_get' or student.id_card like '%$user_get' or student.id_card like '%$user_get%' or student.id_card like '$user_get%'";
                }
                ...
                if ($num_row < 1) {
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        echo '<td>'.$m.'</td>';
                        echo '<td>'.$row["type_student"].'</td>';
                        echo '<td>
                        <center>
                        <a href="edit_regis_data.php?id_card='.$row['barcode_id'].'" class="btn btn-warning" style="margin-left: 0%; width:100%"><span class="glyphicon glyphicon-edit"></span>แก้ไขใบสมัคร</a> 
                        <div style="height:10px"></div>
                        <a href="edit_regis.php?del='.$row['barcode_id'].'" class="btn btn-danger" style="margin-left: 0%; width:100%"><span class="glyphicon glyphicon-trash"></span>ลบใบสมัคร</a>
                        </center>
                        </td></tr></td></tr>';
                        $n++;
                    }
		...
		<table class="table table-responsive table-striped table-bordered  table-hover">
			...
			?>
					<tr>
                        ...
                        <td><?php echo $m?></td>
                        <td><?php echo $row["type_student"]?></td>
						<td><center>
                            <a href="edit_regis_data.php?id_card=<?php echo $row["barcode_id"]?>" class="btn btn-warning" 
                            style="margin-left: 0%;width:100%"><span class="glyphicon glyphicon-edit"></span>แก้ไขใบสมัคร</a> 
                            <div style="height: 10px"></div>
                            <a href="edit_regis.php?del=<?php echo $row['barcode_id'];?>" class="btn btn-danger" 
                            style="margin-left: 0%; width:100%"><span class="glyphicon glyphicon-trash"></span>ลบใบสมัคร</a>
                </center>
                        </td>
					</tr>
                    <?php
